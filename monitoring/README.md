...
Rows: Overview, Calls, SIP traffic, Security, Anti-fraud, Media, Trunks,
Health. The **Media** row answers "did this call carry audio": packets
delivered per leg, media incidents per hour (no anchor, one-way, ports
reused before their quarantine), the endpoint decisions
(`verdict="would-reject"` is counted and **not** enforced — read it before
deciding to enforce) and the RTP port pool.
